...
def load_pretrain(model, pre_s_dict):
    ...
    print(
        "{} keys are not in the pretrain model:".format(len(missing_keys)), missing_keys
    )
    # load new s_dict
...
class Pair_CLIP_SI(nn.Module):
    def __init__(self, node_emb, rel_emb, layer_size, layer_dropout, **kwargs):
        ...
        resnet18_pretrain = torch.load(
            model_simCLR_resnet18_path,
            map_location=torch.device("cpu"),  # changed to cpu
        )
        mlp_pretrain = torch.load(
            model_simCLR_resnet18_path,
            map_location=torch.device("cpu"),  # changed to cpu
        )
        ...
        for i in range(self.num_layer - 1):
            self.layers.append(
                CompGraphConv(self.layer_size[i], self.layer_size[i + 1])
            )
    ...
    def load_checkpoint(self, checkpoint_path):
        """
        If there is a .tar checkpoint file, it is loaded and used to update weights.
        Checkpoint file was created via torch.save(model.state_dict(), out)
...
class Pair_CLIP_SV(nn.Module):
    def __init__(self, node_emb, rel_emb, layer_size, layer_dropout, **kwargs):
        ...
        model_simCLR_resnet18_path = "../data/model_pretrain/checkpoint_100.tar"
        resnet18_pretrain_sv = torch.load(
            model_simCLR_resnet18_path,
            map_location=torch.device("cpu"),  # changed to cpu
        )
        mlp_pretrain_sv = torch.load(
            model_simCLR_resnet18_path,
            map_location=torch.device("cpu"),  # changed to cpu
        )
